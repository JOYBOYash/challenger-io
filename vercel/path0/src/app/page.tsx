...
const proPlanDetails = {
    name: 'Pro',
    description: 'For pros ready to dominate the coding arena.',
    price: 499, // in INR
    currency: 'INR',
};
  ...
  const handlePayment = async () => {
    ...
    try {
      const order = await createOrder({
        amount: proPlanDetails.price * 100, // Amount in paise
        currency: proPlanDetails.currency,
        userId: user.uid,
      });
              ...
              <div className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
                ...
                <div className="relative">
                    ...
                    <div className="cyber-card p-8 flex flex-col h-full border-primary shadow-2xl shadow-primary/10">
                        <div className="flex-1 flex flex-col">
                            <h3 className="text-2xl font-bold font-headline text-primary">{proPlanDetails.name}</h3>
                            <p className="mt-4 text-muted-foreground">{proPlanDetails.description}</p>
                            <div className="mt-6">
                                <span className="text-5xl font-bold">
                                    {new Intl.NumberFormat('en-IN', { style: 'currency', currency: proPlanDetails.currency }).format(proPlanDetails.price)}
                                </span>
                                <span className="text-lg text-muted-foreground">/ month</span>
                            </div>
                            <ul className="mt-8 space-y-4 flex-1">
